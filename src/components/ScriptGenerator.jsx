...
const ScriptGenerator = () => {
  ...
  const handleCopy = () => {
    ...
    const formattedText = `${scriptResult.hook_sentence}${scriptResult.body_script}${scriptResult.cta}`.trim();
      ...
      <div className="main-layout">
        ...
        <div className="card">
          ...
          <div className="form-group">
            ...
            <input type="text" name="productName" value={formData.productName} onChange={handleInputChange} placeholder="Nhập tên sản phẩm..." />
          </div>

          <div className="form-group">
            <label>Khách hàng mục tiêu</label>
            <input type="text" name="targetAudience" value={formData.targetAudience} onChange={handleInputChange} placeholder="Ví dụ: Gen Z, Nhân viên văn phòng..." />
          </div>
          
          <div className="form-group">
             <label>Nền tảng</label>
             <select name="socialNetwork" value={formData.socialNetwork} onChange={handleInputChange}>
          ...
          <div className="form-group">
            ...
            <select name="tone" value={formData.tone} onChange={handleInputChange}>
               ...
               <option value="Cảm động, truyền cảm hứng">Cảm động, truyền cảm hứng</option>
          ...
          <div className="form-group">
            ...
            <textarea name="usp" value={formData.usp} onChange={handleInputChange} rows="3" placeholder="Sản phẩm có gì đặc biệt?" />
          </div>

          <div className="form-group video-upload-area">
            <label style={{cursor: 'pointer'}}>
               <span style={{fontSize: '2rem'}}>📂</span><br/>
               {videoFile ? `Đã chọn: ${videoFile.name}` : "Bấm để tải video lên"}
               <input type="file" accept="video/*" onChange={handleFileChange} style={{display: 'none'}} />
            </label>
            {videoPreview && (
              <video src={videoPreview} controls style={{ width: '100%', marginTop: '15px', borderRadius: '8px' }} />
            )}
          </div>

          <button className="btn-primary" onClick={generateScript} disabled={isLoading}>
            {isLoading ? <><div className="loading-spinner"></div> AI đang xem video...</> : '🚀 Tạo Kịch Bản Ngay'}
          </button>
        ...
        <div className="card result-section">
          ...
          <div className="form-group">
            ...
            <textarea name="hook_sentence" value={scriptResult.hook_sentence} onChange={handleResultEdit} rows="3" />
          </div>

          <div className="form-group">
            <span className="badge badge-body">Nội dung chính</span>
            <textarea name="body_script" value={scriptResult.body_script} onChange={handleResultEdit} rows="10" />
          </div>

          <div className="form-group">
            <span className="badge badge-cta">Call To Action</span>
            <input type="text" name="cta" value={scriptResult.cta} onChange={handleResultEdit} />
          </div>

          <div className="form-group">
            <label>Hashtags</label>
            <div style={{ color: '#4f46e5', fontStyle: 'italic', fontWeight: '500' }}>
              {Array.isArray(scriptResult.caption_hashtags) ? scriptResult.caption_hashtags.join(' ') : scriptResult.caption_hashtags}
            </div>
          </div>
